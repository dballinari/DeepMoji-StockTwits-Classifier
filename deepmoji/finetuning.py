...
def load_benchmark(path, vocab, extend_with=0, save_vocab=False, path_new_vocab='vocabulary_new.json'):
    ...
    # Save extended vocabulary such that we do not need to create the vocabulary each time a prediction is made
    if extend_with > 0 and save_vocab:
        with open(path_new_vocab, 'w') as f:
            json.dump(st.vocabulary, f)
